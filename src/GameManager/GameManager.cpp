...
{
    _gameSystem->execute(
        GenerateSpawnCommand(WarriorBuilder(descr, _map).create_unit(descr.id, descr.hp),
        coord)
    );
}
...
void GameManager::SpawnUnit(const units::ArcherDescription& descr, const map::Point& coord)
{
    ...
    _gameSystem->execute(
        GenerateSpawnCommand(ArcherBuilder(descr, _map).create_unit(descr.id, descr.hp),
        coord)
    );
}
} // sw
